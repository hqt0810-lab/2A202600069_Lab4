...
def search_flights(origin: str, destination: str) -> str:
    ...
    for i, flight in enumerate(flights, 1):
        price_formatted = f"{flight['price']:,}".replace("_", ".")
        result += (
            f"{i}. {flight['airline']}\n"
            f"   Giờ: {flight['departure']} → {flight['arrival']}\n"
            f"   Giá: {price_formatted}đ ({flight['class']})\n"
        )
    return result
...
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    ...
    for i, hotel in enumerate(sorted_hotels, 1):
        price_formatted = f"{hotel['price_per_night']:,}".replace("_", ".")
        stars = "⭐" * hotel['stars']
        result += (
            f"{i}. {hotel['name']} {stars}\n"
            f"   Khu vực: {hotel['area']}\n"
            f"   Giá: {price_formatted}đ/đêm | Rating: {hotel['rating']}/5\n"
        )
    return result
...
def calculate_budget(total_budget: int, expenses: str) -> str:
            ...
            for item in expenses.split(","):
                ...
                if ":" in item:
                    ...
                    try:
                        ...
                        return f"Lỗi: Không thể chuyển đổi '{amount_str}' thành số tiền."
                else:
                    return f"Lỗi: Format sai. Mỗi khoản chi phải có dạng 'tên:số_tiền'."
    except Exception as e:
        return f"Lỗi khi xử lý chi phí: {str(e)}"
    ...
    for name, amount in expenses_dict.items():
        ...
        result += f"- {name.replace('_', ' ')}: {amount_formatted}đ\n"
    
    result += "---\n"
    total_expenses_formatted = f"{total_expenses:,}".replace(",", ".")
    total_budget_formatted = f"{total_budget:,}".replace(",", ".")
    ...
    if remaining >= 0:
        ...
        shortage_formatted = f"{shortage:,}".replace(",", ".")
